...
### **코칭 원칙 (Scaffolding Strategy)**
- **정답 직접 제시 금지**: 학생이 "모르겠다"고 하거나 오답을 말하더라도, 정답이나 특정 정답 선지를 바로 알려주지 마세요.
- **단계적 유도**: 한 번에 너무 많은 정보를 주지 말고, 학생이 스스로 답에 도달할 수 있도록 지문의 특정 부분에 주목하게 하거나 작은 단위의 질문을 던지세요.
- **발견의 기회 제공**: "선지 X번과 지문 Y행을 비교해봐"라고 하기보다, "지문의 @@ 부분에서 ##과 관련된 단서를 찾아볼 수 있을까?"와 같이 학생의 사고를 자극하는 열린 질문을 활용하세요.
- **증거 기반 학습**: 학생이 답을 선택할 때 반드시 지문 내의 근거를 스스로 찾아서 말하도록 유도하세요.
  ...
  msgs.slice(-10).forEach(m => {
    if (m.type === 'student') {
      ...
      if (m.thoughtSignature) {
        msg.extra_content = {
          google: {
            thought_signature: m.thoughtSignature
          }
        };
      }
      result.push(msg);
...
async function callGemini({ apiKey, context, signal }: ProviderArgs): Promise<{ diagnostic: DiagnosticData, thoughtSignature?: string }> {
  ...
  const body = {
    ...
    messages: JSON.parse(context), // context 필드에 JSON화된 messages를 담아 보낼 예정
    temperature: 0.2,
    max_tokens: 8192,
...
⑤본래적 의미의 과징금과 변형된 과징금은 모두, 침익적 행정 행위의 남용을 방지하기 위해 명확한 법적 근거에 따라 부과된다.`,
  ...
  const sendToGemini = useCallback(async (userMessage: string) => {
    ...
    const args: ProviderArgs = {
      ...
      userMessage,
      context: JSON.stringify(geminiMessages),
      signal: abortRef.current?.signal,
    };
    return callGemini(args);
  }, [apiKey, SYSTEM_PROMPT_JSON, currentProblem, messages]);
              ...
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  {currentProblem.difficulty && (
                    <span className={`px-2 py-1 rounded text-xs font-medium ${currentProblem.difficulty === 'easy' ? 'bg-green-100 text-green-800' :
          ...
          {showProblemManager && (
            <div className="space-y-2 mt-3 sm:mt-4">
              {problems.map((problem) => (
                <div
                  key={problem.id}
                  className={`border rounded-lg p-2 sm:p-3 ${selectedProblemId === problem.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className={`flex-1 px-2 py-1 rounded flex items-center justify-center gap-1 text-xs transition-colors ${inputMode === 'text'
                        ...
                        <button
                          ...
                          className={`flex-1 px-2 py-1 rounded flex items-center justify-center gap-1 text-xs transition-colors ${inputMode === 'image'
                            ? 'bg-blue-500 text-white'
                            : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                            }`}
                        >
                      ...
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          ...
                          <div className="flex gap-2 mt-2 flex-wrap">
                            ...
                            {problem.difficulty && (
                              <span className={`px-2 py-0.5 rounded text-xs font-medium ${problem.difficulty === 'easy' ? 'bg-green-100 text-green-800' :
                        ...
                        <div className="flex gap-1 ml-2">
                          <button
                            ...
                            className={`p-1 rounded ${selectedProblemId === problem.id
            ...
            <div className="mt-4 border-2 border-green-300 rounded-lg p-4 sm:p-5 bg-green-50">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 mb-2">
                  <button
                    onClick={() => setInputMode('text')}
                    className={`flex-1 px-3 py-2 rounded flex items-center justify-center gap-2 transition-colors ${inputMode === 'text'
                      ? 'bg-blue-500 text-white'
                      : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                      }`}
                  >
                  ...
                  <button
                    onClick={() => setInputMode('image')}
                    className={`flex-1 px-3 py-2 rounded flex items-center justify-center gap-2 transition-colors ${inputMode === 'image'
                      ? 'bg-blue-500 text-white'
                      : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                      }`}
                  >
            ...
            <div className="h-[500px] sm:h-[550px] lg:h-[600px] overflow-y-auto p-4 sm:p-5 space-y-3 sm:space-y-4">
              ...
              {messages.map((message) => (
                <div key={message.id} className={`flex ${message.type === 'student' ? 'justify-end' : 'justify-start'}`}>
                  <div
                    className={`max-w-[75%] sm:max-w-xs xl:max-w-md rounded-lg p-3 sm:p-4 ${message.isError
                ...
                <div className="border-2 border-purple-200 rounded-lg p-4 sm:p-5 bg-purple-50 mb-4">
                  ...
                  <div className="bg-white rounded p-3 mb-3">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                      <div className="text-black">Survey (훑어보기): <span className="font-medium text-purple-700">{currentDiagnostic.diagnosis.survey_gist}</span></div>
                      <div className="text-black">Question (문항 파악): <span className="font-medium text-purple-700">{currentDiagnostic.diagnosis.question_focus}</span></div>
                      <div className="text-black">Read (정독): <span className="font-medium text-purple-700">{currentDiagnostic.diagnosis.reading_depth}</span></div>
                      <div className="text-black">Recite (재진술): <span className="font-medium text-purple-700">{currentDiagnostic.diagnosis.recite_articulation}</span></div>
                      <div className="text-black">Review (검토): <span className="font-medium text-purple-700">{currentDiagnostic.diagnosis.review_accuracy}</span></div>
